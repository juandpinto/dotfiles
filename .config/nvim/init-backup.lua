...
do
    ...
    vim.keymap.set(
        'n',
        '<leader>e',
        function() MiniFiles.open(vim.uv.cwd()) end
    )
...
do
    ...
    -- ty is not in Mason's registry; use the system-installed binary (e.g. via brew)
    vim.lsp.enable('ty')
end
...
do
    ...
    require('conform').setup({
        ...
        default_format_opts = {
            -- 'always': run LSP formatting in addition to any external formatters below.
            -- (Can't use 'fallback' here because ["*"] being set would suppress LSP on
            -- filetypes that have no explicit formatter entry.)
            lsp_format = 'always',
        },
